...
class Measure(Processor):
    ...
    def build_tagger(self):
        units_en = string_file(get_abs_path("cantonese/data/measure/units_en.tsv"))
        units_zh = string_file(get_abs_path("cantonese/data/measure/units_zh.tsv"))
        units = add_weight((cross("k", "千") | cross("w", "萬")), 0.1).ques + (
            units_en | units_zh
        )
        rmspace = delete(" ").ques
        ...
        number @= self.build_rule(cross("二", "兩"), "[BOS]", "[EOS]")
        # 1-11個，1個-11個
        prefix = number + (rmspace + units).ques + to
        measure = prefix.ques + number + rmspace + units

        for unit in ["兩", "月", "號"]:
            measure @= self.build_rule(cross("兩" + unit, "二" + unit), l="[BOS]")
            measure @= self.build_rule(cross("到兩" + unit, "到二" + unit), r="[EOS]")

        # -xxxx年, -xx年
        digits = Cardinal().digits
        cardinal = digits**2 | digits**4
        unit = accep("年") | accep("年度") | accep("賽季")

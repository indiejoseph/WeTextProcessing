...
class Cardinal(Processor):
    ...
    def __init__(self):
        ...
        # allow simplified -> traditional conversion for rule-level tests
        self.simple_to_traditional = True
        self.number = None
        self.digits = None
        self.build_tagger()
    ...
    def build_tagger(self):
        zero = string_file(get_abs_path("cantonese/data/number/zero.tsv"))
        digit = string_file(get_abs_path("cantonese/data/number/digit.tsv"))
        teen = string_file(get_abs_path("cantonese/data/number/teen.tsv"))
        sign = string_file(get_abs_path("cantonese/data/number/sign.tsv"))
        dot = string_file(get_abs_path("cantonese/data/number/dot.tsv"))
        ...
        number @= self.build_rule(
            cross("二百", "兩百") | cross("二千", "兩千") | cross("二萬", "兩萬"),
            "[BOS]",
        ).optimize()
        percent = insert("百分之") + number + delete("%")
        self.number = accep("約").ques + accep("人均").ques + (number | percent)
        ...
        # In Cantonese, read digits literally (一,二,三...), do not replace 一 with 幺
        phone_digits = digits
        phone = phone_digits**3 | phone_digits**5 | phone_digits**11
        phone |= accep("尾號") + (accep("是") | accep("為")).ques + phone_digits**4

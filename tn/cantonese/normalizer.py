...
from tn.cantonese.rules.char import Char
from tn.cantonese.rules.date import Date
from tn.cantonese.rules.fraction import Fraction
from tn.cantonese.rules.math import Math
from tn.cantonese.rules.measure import Measure
from tn.cantonese.rules.money import Money
from tn.cantonese.rules.postprocessor import PostProcessor
from tn.cantonese.rules.preprocessor import PreProcessor
from tn.cantonese.rules.sport import Sport
from tn.cantonese.rules.time import Time
from tn.cantonese.rules.whitelist import Whitelist
...
class Normalizer(Processor):
    ...
    def __init__(
        ...
        simple_to_traditional=True,
        remove_puncts=False,
        full_to_half=True,
        tag_oov=False,
    ):
        super().__init__(name="yue_normalizer")
        self.remove_interjections = remove_interjections
        self.remove_erhua = remove_erhua
        self.traditional_to_simple = traditional_to_simple
        self.simple_to_traditional = simple_to_traditional
        self.remove_puncts = remove_puncts
        self.full_to_half = full_to_half
        self.tag_oov = tag_oov
        if cache_dir is None:
            cache_dir = files("tn")
        self.build_fst("yue_tn", cache_dir, overwrite_cache)
        # initialize opencc for optional simple->traditional conversion
        if self.simple_to_traditional:
            # use s2hk to prefer Hong Kong traditional variants where applicable
            self._s2t = OpenCC("s2hk")
        else:
            self._s2t = None

    def normalize(self, input):
        if self.simple_to_traditional and self._s2t is not None:
            input = self._s2t.convert(input)
        return super().normalize(input)

    def build_tagger(self):
        # we already apply opencc conversion (Simplified -> Traditional) in normalize(),
        # so avoid applying the Pynini-based simple->traditional mapping which may
        # contain alternative variants that introduce undesired characters.
        processor = PreProcessor(
            traditional_to_simple=self.traditional_to_simple,
            simple_to_traditional=False,
        ).processor
